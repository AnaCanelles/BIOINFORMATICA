...
En esta práctica trabajaremos en el entorno de R, con los paquetes siguientes: `dplyr`, `kableExtra` y `jsonlite`. Para descargar algunos datos de ejemplo, usaremos los programas `datasets` y `dataformat` [@OLeary2024], que pueden descargarse en [este enlace](https://www.ncbi.nlm.nih.gov/datasets/docs/v2/command-line-tools/download-and-install/).
...
Supón que tenemos interés en descargar un genoma de referencia de cualquier especie del género *Trypanosoma*, así como su anotación funcional, en formato [GFF3](https://github.com/The-Sequence-Ontology/Specifications/blob/master/gff3.md). Veamos primero de qué opciones disponemos:

```{bash}
if [ ! -e  ../../data/2026-02-17/trypanosoma_annotated.txt ]; then
   datasets summary genome taxon Trypanosoma --annotated --as-json-lines | \
   dataformat tsv genome --fields \
      accession,organism-name,assminfo-release-date,assminfo-level,assmstats-atgc-count,annotinfo-featcount-gene-total,assminfo-type,assminfo-status > ../../data/2026-02-17/trypanosoma_annotated.txt
fi
```
...
1.  ¿Qué diferencia hay entre las líneas 7 y 8 de la tabla anterior?
    ...
    El identificador de la secuencia es distinto. En la línea 7 es GCF y en la 8 es GCA. Esto es porque provienen de bases de datos distintas, pero tienen la misma información.

2.  Busca una función de R que te permita identificar las líneas repetidas en un marco de datos.

    Podemos consultar el dataframe con `genomas[c(7,8), ]`, que compara las filas 7 y 8 en todas sus columnas, por lo que podemos observar que solo cambia el nombre.
:::
...
```{bash}
if [ ! -e ../../data/2026-02-17/ncbi_data/data/GCA_022059095.1/genomic.gff ]; then
   cd ../../data/2026-02-17
...
La función `summarise` del paquete dplyr muestra el número de variables distintas que hay en cada columnas.
...
A continuación identificamos en cada marco de datos las columnas que tienen solo una variable (son invariables) y los eliminamos:
...
Primero creamos los marcos de datos resumen de cada tipo de la misma manera que antes:
...
Una vez tenemos la tabla resumen de cada marco de datos las combinamos:
...
Partimos de la tabla gene en lugar de la general gff y filtramos comparando con los cromosomas (seqid) candidatos.
...
Para cada cromosoma especificamos que estamos buscando aquellos que empiezan o acaban en el intervalo concreto.
...
La columna 'attributes' contiene el identificador de cada secuencia, que es la única variable única de cada secuencia.
:::
...
1.  ¿Cómo comprobarías que al eliminar el sufijo de la versión, `genomas$acc` sigue siendo una *llave* de la tabla `genomas`?
    ...
    Comprobamos las dimensiones de la tabla (para saber la cantidad de secuencias contenidas) y la cantidad de variables distintas de cada columna. En este caso vemos que la tabla tiene 35 filas y el número de variables distintas en la columna acc también es 35, por lo que podemos considerar que esta sigue siendo una llave de la tabla.
...
2.  Siguiendo el mismo esquema añade a `genomas` el "base_count" y compáralo con el "Assembly.Stats.ATGC.Count". Si observas alguna diferencia, intenta explicarla.
    ...
    En este caso tenemos que tener en cuenta que debemos mantener el argumento match igual, porque las columnas acc y accession son las que tienen las llaves, que es en lo que nos basamos para hacer el match.
